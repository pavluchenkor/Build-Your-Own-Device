...
!!! note "卡片上的自定义传感器和按钮"
    设备卡片不仅限于字典功能（`has*`）。通过卡片清单，固件可以声明任意自定义传感器和控制元件——它们将自动出现在门户网站和应用程序中。具体做法参见通用示例[「智能空气过滤器」](../10-build-a-filter/01-concept.md)，尤其是[卡片章节](../10-build-a-filter/06-card.md)。
