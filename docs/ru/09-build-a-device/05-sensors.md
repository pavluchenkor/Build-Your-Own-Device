...
3. Подключите датчик по I2C (см. [Схему подключения](03-wiring.md)) и читайте его в `main.cpp`:
...
Готового класса термистора для ESP32 в ядре нет, поэтому чтение пишем сами прямо в `main.cpp`. Термистор подключён к выводу ADC через делитель напряжения (см. [Схему подключения](03-wiring.md)): контроллер измеряет напряжение в средней точке, по нему вычисляет сопротивление термистора, а затем — температуру.
...
??? note "Что было — `main.cpp` после главы 4"
