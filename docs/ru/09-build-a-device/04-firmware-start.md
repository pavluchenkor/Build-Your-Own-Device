...
Дальше всё происходит в одном файле — `src/main.cpp`. Откройте его и впишите туда код из этого и следующих шагов.
...
Для нагреваемого шкафа в начало `src/main.cpp` добавьте:
...
В том же `src/main.cpp` после блока `Config` добавьте `setup()` и `loop()`. Для первого запуска достаточно завести Link и крутить его в `loop()`:
...
Соберите оба блока выше в один файл — это всё содержимое `src/main.cpp` на данном шаге:
...
Дальше каждая глава показывает, **что добавить** и **полный `main.cpp` после изменений**, чтобы вы всегда видели целую картину, а не разрозненные куски.
