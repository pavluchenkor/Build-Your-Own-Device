...
Esta secção não está aqui para o criticar.
...
A maioria dos problemas em dispositivos simples acaba por não ser "complexa eletrônica", mas fonte de alimentação, componentes ou ligações incorretas.
...
- invólucro se tornou quente num lugar inesperado;
...
Nesta secção, os erros são divididos em grupos:

- `02-power-mistakes.md` - fonte de alimentação fraca, tensão incorrecta, desfalecimento, fios finos, sem margem.
- `03-wiring-mistakes.md` - polaridade invertida, terminais fracos, sem terra comum, conectores defeituosos.
- `04-controller-mistakes.md` - placa inadequada, lógica 3.3V/5V, GPIO em vez de saída de alimentação, firmware incorrecto.
...
Uma ligação pobre pode funcionar "quase normalmente".
...
Se o dispositivo muda de comportamento quando move o fio, isto não é um erro de software. Este é um problema de ligação.
...
- curto-circuitar o termóstato directamente;
- curto-circuitar a protecção "para teste".

Ideia correcta: procure a razão para a protecção disparar.
...
- A maioria dos erros começa com alimentação, fiação e ligações incorrectas.
...
- A rede AC 110-230V não pode ser diagnosticada "na bancada" sem compreender a segurança eléctrica.
