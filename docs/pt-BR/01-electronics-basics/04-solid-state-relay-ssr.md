...
Em dispositivos simples para impressoras 3D, o SSR é mais frequentemente usado para um aquecedor da rede: câmara, secadora, mesa ou módulo de aquecimento separado. É uma forma conveniente de controlar uma carga `110-230V AC` a partir de eletrônica de baixa tensão, mas não uma forma de tornar a rede segura.

Se o projeto tem `110-230V AC`, a seção de potência deve ser montada num invólucro, com um fusível, terminais normais, isolamento, alívio de tensão do cabo e compreensão da segurança elétrica.

## Como SSR difere de um relé comum

Um relé eletromagnético comum clica porque os contatos se movem fisicamente dentro. A bobina do relé precisa de energia, muitas vezes precisa de um controlador de transistor, diodo de proteção e os contatos podem centelhar, chocalhar e desgastar.
...
- não substitui fusível, invólucro e proteção térmica de emergência.
...
- circuito de supressão ou outra proteção contra picos;
- terminais de potência para a carga.

Em termos simples, é um circuito TRIAC pronto em um invólucro. Portanto, SSR é conveniente onde você não deseja calcular TRIAC optoisolado, TRIAC, supressão, espaçamento de placa e seção de rede sozinho.
...
- aterramento protetor `PE` não é quebrado através do SSR;
- fusível deve proteger a fiação e a carga;
- aquecedor precisa de proteção térmica independente;
- toda a seção de rede deve ser fechada do toque.

O SSR não é considerado um desligador de serviço. Para manutenção, você precisa de uma maneira física de cortar a energia: interruptor, disjuntor, conector ou outra ruptura padrão de circuito. Na prática, pode haver esquemas diferentes de acordo com as regras locais, tipo de potência, aterramento protetor e invólucro. Se você não tem a certeza, uma pessoa qualificada deve verificar o circuito.
...
Modo de falha importante: SSR pode quebrar e ficar ligado. Portanto, proteção térmica de emergência deve ser capaz de cortar a energia do aquecedor independentemente do controlador e SSR.
...
- carga térmica simples sem eletrônica incorporada.
...
Cargas indutivas e eletrônicas podem requerer um tipo diferente de SSR, supressão, varistor, contactor ou circuito separado. Você não pode escolher SSR apenas pelo rótulo `40A`.
...
- corrente máxima no resfriamento necessário;
...
- onde a proteção térmica independente será colocada.

Se não há documentação e o SSR deve controlar um aquecedor da rede, é uma má escolha.
...
- não instalada proteção térmica independente;
...
- [Omron: SSR glossary and installation notes](https://www.ia.omron.com/support/guide/18/further_information.html) - definições de `load voltage`, `leakage current`, `output ON voltage drop`, cruzamento zero, supressão e notas sobre dissipação de calor.
