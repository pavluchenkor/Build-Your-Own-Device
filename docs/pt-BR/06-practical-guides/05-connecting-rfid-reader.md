...
Para projetos simples de impressora 3D, módulos e etiquetas de 13.56 MHz são mais comuns: cartões, fobs, etiquetas compatíveis com NTAG/MIFARE.

## O que verificar antes da conexão

Antes de conectar, procure:
...
- selecção de interface via jumpers ou pontes de soldagem;
...
## RC522: conexão SPI típica

Módulos RC522 baratos geralmente rodam em `3.3V` e geralmente se conectam via SPI.
...
- `MISO` - dados do módulo ao controlador;
- `SDA`, `SS`, or `CS` - SPI chip select;
- `RST` - reset;
- `IRQ` - interrupção, muitas vezes não utilizada em projetos simples.
...
Os nomes dos pinos podem ser diferentes. Por exemplo, no RC522, o pino `SS` geralmente significa `CS`/`SDA` para SPI, não a linha I2C `SDA`. Esta é uma fonte comum de confusão.
...
Mas não pode simplesmente conectar qualquer pino. Em muitas placas PN532, a interface é seleccionada por jumpers, comutadores DIP ou pontes de soldagem.
...
Se a placa diz "lógica 3.3V", não a ligue diretamente a GPIO 5V.

## Massa comum

Como outros módulos, é necessária uma massa comum.

Se o módulo RFID para alimentação de uma fonte e o driver de outra, seus `GND` deverão estar conectados.
...
Para uma bobine de filamento, é melhor colocar o leitor RFID/NFC onde o usuário intencionalmente traz a etiqueta.
...
4. Se UID é conhecido, perfil de material é selecionado.
5. Se UID é desconhecido, dispositivo pede selecção manual de perfil.

RFID não deve ser o único método de controle. Precisa de uma cópia de segurança manual: perfil em menu, botão, display ou configuração de interface.

## O que verificar após montagem

Verifique:

- módulo recebe tensão correta;
- níveis de lógica são compatíveis com controlador;
- interface correta é seleccionada;
- `MOSI`, `MISO`, `SCK`, `CS` não estão trocados para SPI;
- `SDA`, `SCL` não estão trocados para I2C;
- `TX` e `RX` estão corretamente cruzados para UART;
- massa comum existe;
- reset/IRQ estão conectados conforme a biblioteca requer;
- etiquetas do tipo correto leem;
...
- conectar RC522 3.3V a alimentação 5V ou lógica 5V;
- confundir RC522 `SDA` com I2C `SDA`;
- esquecer `CS`/`SS` em SPI;
- trocar `MOSI` e `MISO`;
- selecionar uma interface em PN532 com jumpers mas fiação outra;
