...
まずカードの構造から。カードは**行**の垂直スタックです。行とは1〜4個のエンティティを持つ水平帯で、カードの幅を均等に分割します: 1つなら全幅、2つなら半分ずつ、3つなら3分の1ずつ。
...
│  103              │  オフ       │
├───────────────────┼─────────────┤
│  Mode      [auto ▾] │ Threshold [150] │   ← 行2: mode, threshold
└─────────────────────────────────┘
```
...
- **ファン**セル — オン/オフ;
- **Mode** — ドロップダウンリスト、**VOC threshold** — 送信ボタン付きフィールド。
